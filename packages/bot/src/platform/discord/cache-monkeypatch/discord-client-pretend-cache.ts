...
const pretendCaches: Record<string, PretendCache<any, any>> = {};
...
export class PretendCache<Key= any, Value= any> implements Collection<Key, Value> {
  public static makeOne(label: string) {
    return pretendCaches[label] ??= new PretendCache(label);
  }
  constructor(public readonly label: string) {
  }

  ["constructor"]: CollectionConstructor = ((label, ...idk: any[]) => PretendCache.makeOne(label)) as any;

  ensure(key: Key, defaultValueGenerator: (key: Key, collection: this) => Value): Value {
    GLOBAL_STATE.isAfterStartup && console.log(`[${this.label}]`);
